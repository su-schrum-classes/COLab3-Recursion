...
Given the above explanation, write a `sumRecursive` procedure that computes the sum of all integers from 1 up to and including some integer parameter.

Finally, write `factorialRecursive`, which will require you to use the assembly multiplication instruction. There is an example of how to use `MUL` for 32-bit unsigned integers under the label `mulExample`.
